...
def create_training_grid_with_titles(images, class_names, epoch, guidance_scale, image_size):
    ...
    # Create figure with proper spacing for titles
    ...
    for i in range(4):
        for j in range(4):
            ...
            if idx < len(class_names):
                ax.set_title(f'{class_names[idx]}', fontsize=9, fontweight='bold', pad=5)
    
    plt.tight_layout()
    return fig

def generate_samples(model, path, device, epoch, samples_dir, config, num_samples=16):
    ...
    with torch.no_grad():
        ...
        # Sample from first 16 classes (0-15) for monitoring
        ...
        sample_file = os.path.join(samples_dir, f"samples_epoch_{epoch:04d}_classes_0-15.png")
        fig.savefig(sample_file, dpi=150, bbox_inches='tight')
        plt.close(fig)
        
        # Log detailed class information
        class_info = [f"{i}: {class_names[i]}" for i in range(16)]
        class_summary = f"Classes 0-15: {', '.join([class_names[i] for i in range(16)])}"
        ...
        with open(class_mapping_file, 'w') as f:
            f.write(f"Sample grid layout (4x4):\n")
            f.write(f"Row 1: Classes 0-3\n")
            f.write(f"Row 2: Classes 4-7\n")
            f.write(f"Row 3: Classes 8-11\n")
            f.write(f"Row 4: Classes 12-15\n\n")
            f.write(f"Detailed mapping:\n")
            for i in range(16):
                f.write(f"Position {i}: Class {i} - {class_names[i]}\n")
