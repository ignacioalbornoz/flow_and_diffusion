...
def create_training_grid_with_titles(images, class_names, epoch, guidance_scale, image_size):
    ...
    # Create figure with proper spacing for titles (8x8 for 62 classes, showing first 64)
    ...
    for i in range(8):
        for j in range(8):
            ...
            if idx < num_samples:
                ...
                ax.set_title(f'{class_names[class_idx]}', fontsize=8, fontweight='bold', pad=2)
            else:
                ax.axis('off')
...
def generate_samples(model, path, device, epoch, samples_dir, config, num_samples=64):
    ...
    with torch.no_grad():
        ...
        # Sample from available classes (62 classes for 62-class setup)
        num_classes = len(class_names)
        ...
        # Create grid with titles (8x8 for 62 classes)
        ...
        class_summary = f"Classes 0-{num_classes-1}: {', '.join([class_names[i] for i in range(min(10, num_classes))])}..."
        ...
        with open(class_mapping_file, 'w') as f:
            f.write(f"Sample grid layout (8x8):\n")
            f.write(f"Showing first 64 samples from {num_classes} classes\n\n")
            f.write(f"Detailed mapping:\n")
            for i in range(min(64, num_samples)):
...
def main(experiment_name: str):
    ...
    try:
        ...
        data_dir = config.get("data_dir", "/home/ialbornoz/tesis/flow_and_diffusion/sketch/data/sketch")
        logger.info(f"Using dataset directory: {data_dir}")
        path = util.GaussianConditionalProbabilityPath(
            p_data = sketch_model.SketchSampler(data_dir=data_dir),
        ...
        losses = []
        best_loss = float('inf')
        patience = 2000  # Early stopping patience (20% of total epochs)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train 62-class sketch diffusion model')
    parser.add_argument('experiment_name', help='Name of the experiment to run')
    parser.add_argument('--data_dir', help='Path to the dataset directory with 62 classes', 
                       default="/home/ialbornoz/tesis/flow_and_diffusion/sketch/data/sketch")
    ...
    config = configs[args.experiment_name]
