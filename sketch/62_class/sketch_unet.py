...
class FourierEncoder(nn.Module):
    ...
    def forward(self, t: torch.Tensor) -> torch.Tensor:
        ...
        """
        t = t.view(-1, 1) # (bs, 1)
        freqs = t * self.weights.to(t.device) * 2 * math.pi # (bs, half_dim)
...
class SketchUNet(util.ConditionalVectorField):
    def __init__(self, channels: List[int], num_residual_layers: int, t_embed_dim: int, y_embed_dim: int): 
        ...
        # Initialize y embedder for 63 classes (62 + 1 null class)
        self.y_embedder = nn.Embedding(num_embeddings = 63, embedding_dim = y_embed_dim)  # 62 classes + 1 null class
        # Register a dummy parameter to ensure the model is moved to the correct device
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, y: torch.Tensor):
        ...
        y = y.to(self.y_embedder.weight.device)  # Ensure y is on the same device as embedding
