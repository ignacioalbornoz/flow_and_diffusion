...
class CFGVectorFieldODE(ODE):
    ...
    def drift_coefficient(self, x: torch.Tensor, t: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        unguided_y = torch.ones_like(y) * 2  # Updated to match null class for sketches (2 classes + 1 null)
...
class CFGTrainer(Trainer):
    ...
    def get_train_loss(self, batch_size: int) -> torch.Tensor:
        ...
        # Step 2: Set each label to 2 (i.e., null) with probability eta
        xi = torch.rand(y.shape[0]).to(y.device)
        y[xi < self.eta] = 2.0  # Set to null class (2 for 2 classes + 1 null)
