...
def create_experiment_configs():
    ...
    configs = {
        "sketch_32x32_50classes": {
            "description": "32x32 resolution, 50 classes, research-grade model",
            "image_size": 32,
        ...
        "sketch_32x32_v3_improved": {
            "description": "32x32 resolution, 50 classes, SD v3 improved architecture",
            "image_size": 32,
            "model_channels": [96, 192, 384, 768],  # Larger channels for better capacity
            "num_residual_layers": 6,  # More layers for deeper model
            "learning_rate": 1e-4,  # Same learning rate as original
            "guidance_scale": 1.5,  # Same guidance scale as original
            "eta": 0.1,  # Same eta as original
            "weight_multiplier": 1.0,  # No weighting
            "batch_size": 32,  # Same batch size as original
            "num_epochs": 10000,  # Same 10000 epochs as original
            "checkpoint_interval": 50,  # Same checkpoint interval as original
            "sample_interval": 50,  # Same sample interval as original
            "use_v3_model": True  # Flag to use the improved model
        },
        "sketch_32x32_research": {
            "description": "32x32 resolution, research-grade model (like MNIST but larger)",
            "image_size": 32,
            "model_channels": [64, 128, 256, 512],  # Much larger model
            "num_residual_layers": 4,  # More layers
            "learning_rate": 1e-4,  # Lower learning rate for stability
