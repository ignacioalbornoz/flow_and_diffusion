...
class CFGVectorFieldODE(ODE):
    ...
    def drift_coefficient(self, x: torch.Tensor, t: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        unguided_y = torch.ones_like(y) * 50  # Updated to match null class for sketches (50 classes + 1 null)
...
class CFGTrainer(Trainer):
    ...
    def get_train_loss(self, batch_size: int) -> torch.Tensor:
        ...
        # Step 2: Set each label to 50 (i.e., null) with probability eta
        xi = torch.rand(y.shape[0]).to(y.device)
        y[xi < self.eta] = 50.0  # Set to null class (50 for 50 classes + 1 null)
        ...
        # Step 4: Regress and output loss - ensure all tensors are on the same device
        device = next(self.model.parameters()).device
        x = x.to(device)
        t = t.to(device)
        y = y.to(device)
        ...
        # Step 5: Standard MSE loss without weighting
        squared_error = torch.square(ut_theta - ut_ref)  # (bs, 1, 32, 32)
        error = torch.einsum('bchw -> b', squared_error) # (bs,)
