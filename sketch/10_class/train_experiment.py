...
def create_training_grid_with_titles(images, class_names, epoch, guidance_scale, image_size):
    ...
    # Create figure with proper spacing for titles (5x2 for 10 classes)
    ...
    for i in range(5):
        for j in range(2):
            ...
            if idx < num_samples:
                ...
                ax.set_title(f'{class_names[class_idx]}', fontsize=10, fontweight='bold', pad=3)
            else:
                ax.axis('off')
...
def generate_samples(model, path, device, epoch, samples_dir, config, num_samples=10):
    ...
    with torch.no_grad():
        ...
        # Sample from available classes (10 classes for 10-class setup)
        num_classes = len(class_names)
        ...
        # Create grid with titles (5x2 for 10 classes)
        ...
        with open(class_mapping_file, 'w') as f:
            f.write(f"Sample grid layout (5x2):\n")
            f.write(f"Row 1: Classes 0-1\n")
            f.write(f"Row 2: Classes 2-3\n")
            f.write(f"Row 3: Classes 4-5\n")
            f.write(f"Row 4: Classes 6-7\n")
            f.write(f"Row 5: Classes 8-9\n\n")
